...
public class RegistServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("success".equals(result)) {
            // 注册成功，重定向到注册成功页面
            response.sendRedirect(request.getContextPath() + "/view/member/register_success.html");
        } else {
            // 注册失败，设置错误消息并返回到注册页面
            response.sendRedirect(request.getContextPath() + "/view/member/register_fail.html");
        }
    }
}